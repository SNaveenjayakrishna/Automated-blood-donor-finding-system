...
const AvailableDonors = ({id}) => {
 ...
 // const [filteredDonors, setFilteredDonors] = useState([]);
  ...
  const fetchDonors = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/available_donors/${id}`, {
        method: "GET",
        credentials: "include"
      });
  ...
  {/*useEffect(() => {
    const filtered = donors.filter((donor) =>
      ...
      donor.mobile.includes(searchQuery) ||
      donor.blood_group.includes(searchQuery)
    );
  ...
  const updateDonation = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDonors((prev) => {
          return prev.map((donor) => {
            if (donor.donor_id === user_id) {
              donor.is_certificate_issued = true;
            }
            return donor;
          });
        });
  ...
  if (loading)
    return <p className={cardStyles.card}>Loading donors...</p>;
      ...
      {selectedDonor && (
        ...
        >
          ...
          >
            ...
            <p><b>Address:</b> {selectedDonor.full_address}</p>
           
            <p>
              <b>Responded at:</b>{" "}
              {selectedDonor.responded_at
                ? selectedDonor.responded_at
                : "—"}
            </p>
          ...
          {!selectedDonor.is_certificate_issued && <div>
          <label style={{ display: "block", marginBottom: "5px" }}>
              Donation Date
          </label>

            <input type="date" className={inputStyles.input} value={donationDate} onChange={(e) => setDonationDate(e.target.value)}
          ...
          >
            {selectedDonor.is_certificate_issued? "Donated": "Confirm Donation" }
          </button>
