...
const PendingHospitals = () => {
    ...
    const [showReject, setShowReject] = useState(false);
      ...
      try {
        const res = await fetch(`${BACKEND_URL}/admin/pending_hospitals`, { method: "GET", credentials: "include" });
...
const handleReject = async (id) => {
  ...
  try{
  const response = await fetch(`${BACKEND_URL}/admin/reject_hospital/${id}`, {
    method: "POST",
...
<p>
  ...
  >
    ...
    <div style={{ marginTop: "15px" }}>
      ...
      {selectedHospital.current_status === "pending" && <button
    ...
    <button
      className={`${buttonStyles.btn} ${buttonStyles["btn-danger"]}`}
      style={{ marginLeft: "10px" }}
      onClick={() => handleReject(selectedHospital.id)}
    >
      Confirm Rejection
    </button>
  </div>
